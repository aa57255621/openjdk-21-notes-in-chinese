...
void HeapRegion::setup_heap_region_size(size_t max_heap_size) {
  // 1. 初始区域大小为G1HeapRegionSize的默认值。
  size_t region_size = G1HeapRegionSize;

  // 2. 如果G1HeapRegionSize为0，则表示应该根据最大堆大小和目标区域数量来决定区域大小。
  if (region_size == 0) {
    // 使用clamp函数确保区域大小在最小和最大堆尺寸之间。
    region_size = clamp(max_heap_size / HeapRegionBounds::target_number(),
                        HeapRegionBounds::min_size(),
                        HeapRegionBounds::max_ergonomics_size());
  }

  // 3. 确保区域大小是2的幂。通常情况下，向上取整是有利的，因为这样可以减少内存碎片，
  //    并且与G1垃圾回收器的其他部分更好地协调。
  region_size = round_up_power_of_2(region_size);

  // 4. 再次使用clamp函数确保区域大小在最小和最大尺寸之间。
  //    这有助于确保我们不会超出JVM堆的最大大小限制。
  ...
  // 6. 设置全局变量，这些变量将用于G1垃圾回收器的其他部分。
  //    我们使用guarantee函数来确保这些变量只设置一次，以避免重复设置导致的错误。
  ...
  // 7. 如果G1HeapRegionSize与计算出的GrainBytes不同，则更新G1HeapRegionSize以匹配。
  //    这确保了我们的配置与实际计算出的区域大小一致。
